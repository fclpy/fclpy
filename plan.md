...
| C12 | **Reader** — `reader/` 136 failing of 165 (**17.6%**). `#(1 2 3)` reads as the cons `(VECTOR 1 2 3)` (CLHS 2.4.8.3); the tokenizer interprets `\n` inside strings, where CLHS 2.4.5 requires backslash to be a single-escape included *without interpretation*. ~~**Also: `fclpy/reader.py` was a dead ~480-line second reader** that nothing under `fclpy/` imported, yet **177 unit tests (14% of that suite)** certified it.~~ **Done 2026-09-01:** all four files repointed at the live reader via `conftest.py`'s `read`/`read_all`/`read_in_package`, and `fclpy/reader.py` deleted. Five tests had been asserting the dead reader's *bugs* as correct (`#(...)` as a `(VECTOR ...)` call form, `,foo` outside a backquote as `(UNQUOTE foo)`, `123abc` as the integer 123, `#()` printing as `(VECTOR)`) and now assert the ANSI answer; both `xfail(strict)` rows are gone, and one test's `while` loop that never advanced its cursor was fixed. | M10 |
